...
$$
DP[p][c] = max\{ DP[p][c], DP [p-1][c'] + w_p s_i \}
$$
Sous contraintes :
* $i$ n'a pas encore été utlisé 
* $c=c'+b(i)$
* $c'_g \le \alpha_g \quad \forall g$

avec 

* $p$ : la position
* $c=(c[1], c[2], ..., c[G])$ : vecteur des compteurs par groupe
* $DP[p][\alpha]$ : meilleur score pour avoir sélectionné p éléments avec configuration $c$.
...
De même, si le paramètre beam_size est trop faible, la solution trouvée par l'algorithme de programmation dynamique avec tas n'est pas optimale :
...
Remarque : Un beam_size égal à revient au même que l'algorithme naïf, on garde le meilleur état à chaque étape. 
...
$$\boxed{T_{\text{DP}}(n) = O(n\bar{g}) + O(BG(nk + k^2))}$$
...
| **Beam C++**         | $O(B(nk + k^2))$        | $O(B\bar{g}(nk + k^2))$| $O(BG(nk + k^2))$           |
B * G * (n * k + k^2)
...
df_impact <- data.frame(
  ...
  # Beam
  Beam_moyen = B * g2_bar * (n * k + k^2),
  Beam_pire  = B * G_values * (n * k + k^2)
)
...
- **Programmation dynamique (C++)** : la pente plus faible (~1.7) reflète une complexité mieux contrôlée. 

- **Programmation dynamique avec tas (C++)** : la croissance du temps reste modérée avec une pente d'environ 1.27, même pour de grandes valeurs de \( n \). Cependant, comme l'algorithme naïf, l'optimalité n'est pas garantit. 
...
|----------|----------------------|---------------|
| Petits problèmes (n<100, G<3, k<10) | **Naïf R** ou **DP C++** ou **Beam C++** | Rapide, optimal (garanti pour DP C++) |
| Moyens problèmes (n<1000, G<5, k<20) | **Naïf R** ou **DP C++** ou **Beam C++**| Optimal (garanti pour DP C++), très bonne performance |
| Grands problèmes (n>1000 ou G>5) | **Naïf R** | Le plus rapide, quasi-optimal |
| Recherche d'optimum prouvé | **DP C++** si faisable | Exact et plus rapide que **DP R** |
